        ...
        <?php if (!empty($all_cards_indices)): ?>
            ...
            function build_slots($config_layers, $center_x = 50, $center_y = 50) {
                ...
                foreach ($config_layers as $layer) {
                    ...
                    if ($r == 0) {
                        // Это ЦЕНТРАЛЬНЫЙ слой (радиус 0)
                        for ($i = 0; $i < $c; $i++) { // Обычно count=1 для центра
                            $slots[] = [
                                'size' => $s,
                                'top' => $center_y - ($s / 2),
                                'left' => $center_x - ($s / 2),
                                'z_index' => 10 - $layer_index // Центральные слои выше
                            ];
                        }
                    } else {
                        // Это ОРБИТАЛЬНЫЙ слой
                        $angle_step = (M_PI * 2) / $c;
                        // Случайное смещение угла для каждой генерации и каждого слоя, чтобы макеты не выглядели одинаково
            ...
            function getSymbolLayoutSlots($k) {
                ...
                $all_recipes[8] = [
                    // Рецепт A: 1 большой в центре, 7 маленьких на орбите
                    [ ['size' => 42, 'count' => 1, 'radius' => 0], ['size' => 25, 'count' => 7, 'radius' => 35.5] ],
                    // Рецепт Б: 1 средний в центре, 7 средних на орбите
                    [ ['size' => 38, 'count' => 1, 'radius' => 0], ['size' => 28, 'count' => 7, 'radius' => 34] ],
                    // Рецепт В: Нет центра, 8 одинаковых на одной орбите
                    [ ['size' => 26, 'count' => 8, 'radius' => 35] ],
                    // Рецепт Г: Две орбиты (4 маленьких внутри, 4 больших снаружи)
                    [ ['size' => 16, 'count' => 4, 'radius' => 11.5], ['size' => 28, 'count' => 4, 'radius' => 34] ]
                ];
                
                // --- K = 6 (Порядок 5, 6 слотов) ---
                $all_recipes[6] = [
                    // A: 1 маленький центр, 5 больших на орбите
                    [ ['size' => 26, 'count' => 1, 'radius' => 0], ['size' => 34, 'count' => 5, 'radius' => 31] ],
                    // Б: 1 большой центр, 5 маленьких на орбите
                    [ ['size' => 40, 'count' => 1, 'radius' => 0], ['size' => 25, 'count' => 5, 'radius' => 35.5] ],
                    // В: Нет центра, 6 на одной орбите
                    [ ['size' => 32, 'count' => 6, 'radius' => 32] ]
                ];

                // --- K = 5 (Порядок 4, 5 слотов) ---
                $all_recipes[5] = [
                    // A: 1 маленький центр, 4 больших на орбите
                    [ ['size' => 18, 'count' => 1, 'radius' => 0], ['size' => 38, 'count' => 4, 'radius' => 29] ],
                    // Б: 1 большой центр, 4 маленьких на орбите
                    [ ['size' => 38, 'count' => 1, 'radius' => 0], ['size' => 28, 'count' => 4, 'radius' => 34] ],
                    // В: Нет центра, 5 на одной орбите
                    [ ['size' => 35, 'count' => 5, 'radius' => 30.5] ]
                ];
                
                // --- K = 4 (Порядок 3, 4 слота) ---
                $all_recipes[4] = [
                    // A: 1 крошечный центр, 3 огромных на орбите
                    [ ['size' => 10, 'count' => 1, 'radius' => 0], ['size' => 42, 'count' => 3, 'radius' => 27] ],
                    // Б: 1 большой центр, 3 маленьких на орбите
                    [ ['size' => 36, 'count' => 1, 'radius' => 0], ['size' => 30, 'count' => 3, 'radius' => 33] ],
                    // В: Нет центра, 4 на одной орбите
                    [ ['size' => 39, 'count' => 4, 'radius' => 28.5] ]
                ];
                
                // --- K = 3 (Порядок 2, 3 слота) ---
                // Используем как значение по умолчанию
                $all_recipes[3] = [
                    // A: Сбалансированный (1 центр, 2 на орбите)
                    [ ['size' => 30, 'count' => 1, 'radius' => 0], ['size' => 30, 'count' => 2, 'radius' => 31.5] ],
                    // Б: Нет центра, 3 на одной орбите
                    [ ['size' => 44, 'count' => 3, 'radius' => 26] ]
                ];
